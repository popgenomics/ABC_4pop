...
subsampled = int(sys.argv[1]) # Nref : 100000
...
header_bpfile = infile.readline() # first empty line
...
subsampled_bin = sample(range(nLoci), subsampled)
subsampled_bin.sort()
...
#					DtajD.append(tajD(tmpD['pi'], thetaD_locus, nSamD[nLoci_cnt - 1], tmp['sxD']+tmp['ss'], a1_spD[nLoci_cnt-1], a2_spD[nLoci_cnt-1]))
	...
	if nLoci_cnt != 0 and len(sxA) == nLoci:
		...
		nLoci_cnt = 0
		# statistics
		bialsites_avg = cr_mean([ bialsites[j] for j in subsampled_bin])
		bialsites_std = cr_std([ bialsites[j] for j in subsampled_bin], bialsites_avg)
		sfAB_avg = cr_mean([ sfAB[j] for j in subsampled_bin])
		sfAB_std = cr_std([ sfAB[j] for j in subsampled_bin], sfAB_avg)
		sfAC_avg = cr_mean([ sfAC[j] for j in subsampled_bin])
		sfAC_std = cr_std([ sfAC[j] for j in subsampled_bin], sfAC_avg)
		sfAD_avg = cr_mean([ sfAD[j] for j in subsampled_bin])
		sfAD_std = cr_std([ sfAD[j] for j in subsampled_bin], sfAD_avg)
		sfBC_avg = cr_mean([ sfBC[j] for j in subsampled_bin])
		sfBC_std = cr_std([ sfBC[j] for j in subsampled_bin], sfBC_avg)
		sfBD_avg = cr_mean([ sfBD[j] for j in subsampled_bin])
		sfBD_std = cr_std([ sfBD[j] for j in subsampled_bin], sfBD_avg)
		sfCD_avg = cr_mean([ sfCD[j] for j in subsampled_bin])
		sfCD_std = cr_std([ sfCD[j] for j in subsampled_bin], sfCD_avg)
		
		sxA_avg = cr_mean([ sxA[j] for j in subsampled_bin])
		sxA_std = cr_std([ sxA[j] for j in subsampled_bin], sxA_avg)
		sxB_avg = cr_mean([ sxB[j] for j in subsampled_bin])
		sxB_std = cr_std([ sxB[j] for j in subsampled_bin], sxB_avg)
		sxC_avg = cr_mean([ sxC[j] for j in subsampled_bin])
		sxC_std = cr_std([ sxC[j] for j in subsampled_bin], sxC_avg)
		sxD_avg = cr_mean([ sxD[j] for j in subsampled_bin])
		sxD_std = cr_std([ sxD[j] for j in subsampled_bin], sxD_avg)
	
		ssAB_avg = cr_mean([ ssAB[j] for j in subsampled_bin])
		ssAB_std = cr_std([ ssAB[j] for j in subsampled_bin], ssAB_avg)
		ssAC_avg = cr_mean([ ssAC[j] for j in subsampled_bin])
		ssAC_std = cr_std([ ssAC[j] for j in subsampled_bin], ssAC_avg)
		ssAD_avg = cr_mean([ ssAD[j] for j in subsampled_bin])
		ssAD_std = cr_std([ ssAD[j] for j in subsampled_bin], ssAD_avg)
		ssBC_avg = cr_mean([ ssBC[j] for j in subsampled_bin])
		ssBC_std = cr_std([ ssBC[j] for j in subsampled_bin], ssBC_avg)
		ssBD_avg = cr_mean([ ssBD[j] for j in subsampled_bin])
		ssBD_std = cr_std([ ssBD[j] for j in subsampled_bin], ssBD_avg)
		ssCD_avg = cr_mean([ ssCD[j] for j in subsampled_bin])
		ssCD_std = cr_std([ ssCD[j] for j in subsampled_bin], ssCD_avg)
		
		#successive_ss_avg = cr_mean(successive_ss)
		#successive_ss_std = cr_std(successive_ss, successive_ss_avg)
		
		piA_avg = cr_mean([ piA[j] for j in subsampled_bin])
		piA_std = cr_std([ piA[j] for j in subsampled_bin], piA_avg)
		piB_avg = cr_mean([ piB[j] for j in subsampled_bin])
		piB_std = cr_std([ piB[j] for j in subsampled_bin], piB_avg)
		piC_avg = cr_mean([ piC[j] for j in subsampled_bin])
		piC_std = cr_std([ piC[j] for j in subsampled_bin], piC_avg)
		piD_avg = cr_mean([ piD[j] for j in subsampled_bin])
		piD_std = cr_std([ piD[j] for j in subsampled_bin], piD_avg)
		
		pearson_r_pi_AB = cr_pearsonR([ piA[j] for j in subsampled_bin], [ piB[j] for j in subsampled_bin])
		pearson_r_pi_AC = cr_pearsonR([ piA[j] for j in subsampled_bin], [ piC[j] for j in subsampled_bin])
		pearson_r_pi_AD = cr_pearsonR([ piA[j] for j in subsampled_bin], [ piD[j] for j in subsampled_bin])
		pearson_r_pi_BC = cr_pearsonR([ piB[j] for j in subsampled_bin], [ piC[j] for j in subsampled_bin])
		pearson_r_pi_BD = cr_pearsonR([ piB[j] for j in subsampled_bin], [ piD[j] for j in subsampled_bin])
		pearson_r_pi_CD = cr_pearsonR([ piC[j] for j in subsampled_bin], [ piD[j] for j in subsampled_bin])
		
		thetaA_avg = cr_mean([ thetaA[j] for j in subsampled_bin])
		thetaA_std = cr_std([ thetaA[j] for j in subsampled_bin], thetaA_avg)
		thetaB_avg = cr_mean([ thetaB[j] for j in subsampled_bin])
		thetaB_std = cr_std([ thetaB[j] for j in subsampled_bin], thetaB_avg)
		thetaC_avg = cr_mean([ thetaC[j] for j in subsampled_bin])
		thetaC_std = cr_std([ thetaC[j] for j in subsampled_bin], thetaC_avg)
		thetaD_avg = cr_mean([ thetaD[j] for j in subsampled_bin])
		thetaD_std = cr_std([ thetaD[j] for j in subsampled_bin], thetaD_avg)
		
		pearson_r_theta_AB = cr_pearsonR([ thetaA[j] for j in subsampled_bin], [ thetaB[j] for j in subsampled_bin])
		pearson_r_theta_AC = cr_pearsonR([ thetaA[j] for j in subsampled_bin], [ thetaC[j] for j in subsampled_bin])
		pearson_r_theta_AD = cr_pearsonR([ thetaA[j] for j in subsampled_bin], [ thetaD[j] for j in subsampled_bin])
		pearson_r_theta_BC = cr_pearsonR([ thetaB[j] for j in subsampled_bin], [ thetaC[j] for j in subsampled_bin])
		pearson_r_theta_BD = cr_pearsonR([ thetaB[j] for j in subsampled_bin], [ thetaD[j] for j in subsampled_bin])
		pearson_r_theta_CD = cr_pearsonR([ thetaC[j] for j in subsampled_bin], [ thetaD[j] for j in subsampled_bin])
		
		DtajA_avg = cr_mean([ DtajA[j] for j in subsampled_bin])
		DtajA_std = cr_std([ DtajA[j] for j in subsampled_bin], DtajA_avg)
		DtajB_avg = cr_mean([ DtajB[j] for j in subsampled_bin])
		DtajB_std = cr_std([ DtajB[j] for j in subsampled_bin], DtajB_avg)
		DtajC_avg = cr_mean([ DtajC[j] for j in subsampled_bin])
		DtajC_std = cr_std([ DtajC[j] for j in subsampled_bin], DtajC_avg)
		DtajD_avg = cr_mean([ DtajD[j] for j in subsampled_bin])
		DtajD_std = cr_std([ DtajD[j] for j in subsampled_bin], DtajD_avg)
		
		divAB_avg = cr_mean([ divAB[j] for j in subsampled_bin])
		divAB_std = cr_std([ divAB[j] for j in subsampled_bin], divAB_avg)
		divAC_avg = cr_mean([ divAC[j] for j in subsampled_bin])
		divAC_std = cr_std([ divAC[j] for j in subsampled_bin], divAC_avg)
		divAD_avg = cr_mean([ divAD[j] for j in subsampled_bin])
		divAD_std = cr_std([ divAD[j] for j in subsampled_bin], divAD_avg)
		divBC_avg = cr_mean([ divBC[j] for j in subsampled_bin])
		divBC_std = cr_std([ divBC[j] for j in subsampled_bin], divBC_avg)
		divBD_avg = cr_mean([ divBD[j] for j in subsampled_bin])
		divBD_std = cr_std([ divBD[j] for j in subsampled_bin], divBD_avg)
		divCD_avg = cr_mean([ divCD[j] for j in subsampled_bin])
		divCD_std = cr_std([ divCD[j] for j in subsampled_bin], divCD_avg)
		
		netdivAB_avg = cr_mean([ netdivAB[j] for j in subsampled_bin])
		netdivAB_std = cr_std([ netdivAB[j] for j in subsampled_bin], netdivAB_avg)
		netdivAC_avg = cr_mean([ netdivAC[j] for j in subsampled_bin])
		netdivAC_std = cr_std([ netdivAC[j] for j in subsampled_bin], netdivAC_avg)
		netdivAD_avg = cr_mean([ netdivAD[j] for j in subsampled_bin])
		netdivAD_std = cr_std([ netdivAD[j] for j in subsampled_bin], netdivAD_avg)
		netdivBC_avg = cr_mean([ netdivBC[j] for j in subsampled_bin])
		netdivBC_std = cr_std([ netdivBC[j] for j in subsampled_bin], netdivBC_avg)
		netdivBD_avg = cr_mean([ netdivBD[j] for j in subsampled_bin])
		netdivBD_std = cr_std([ netdivBD[j] for j in subsampled_bin], netdivBD_avg)
		netdivCD_avg = cr_mean([ netdivCD[j] for j in subsampled_bin])
		netdivCD_std = cr_std([ netdivCD[j] for j in subsampled_bin], netdivCD_avg)
		
		minDivAB_avg = cr_mean([ minDivAB[j] for j in subsampled_bin])
		minDivAB_std = cr_std([ minDivAB[j] for j in subsampled_bin], minDivAB_avg)
		minDivAC_avg = cr_mean([ minDivAC[j] for j in subsampled_bin])
		minDivAC_std = cr_std([ minDivAC[j] for j in subsampled_bin], minDivAC_avg)
		minDivAD_avg = cr_mean([ minDivAD[j] for j in subsampled_bin])
		minDivAD_std = cr_std([ minDivAD[j] for j in subsampled_bin], minDivAD_avg)
		minDivBC_avg = cr_mean([ minDivBC[j] for j in subsampled_bin])
		minDivBC_std = cr_std([ minDivBC[j] for j in subsampled_bin], minDivBC_avg)
		minDivBD_avg = cr_mean([ minDivBD[j] for j in subsampled_bin])
		minDivBD_std = cr_std([ minDivBD[j] for j in subsampled_bin], minDivBD_avg)
		minDivCD_avg = cr_mean([ minDivCD[j] for j in subsampled_bin])
		minDivCD_std = cr_std([ minDivCD[j] for j in subsampled_bin], minDivCD_avg)
		
		maxDivAB_avg = cr_mean([ maxDivAB[j] for j in subsampled_bin])
		maxDivAB_std = cr_std([ maxDivAB[j] for j in subsampled_bin], maxDivAB_avg)
		maxDivAC_avg = cr_mean([ maxDivAC[j] for j in subsampled_bin])
		maxDivAC_std = cr_std([ maxDivAC[j] for j in subsampled_bin], maxDivAC_avg)
		maxDivAD_avg = cr_mean([ maxDivAD[j] for j in subsampled_bin])
		maxDivAD_std = cr_std([ maxDivAD[j] for j in subsampled_bin], maxDivAD_avg)
		maxDivBC_avg = cr_mean([ maxDivBC[j] for j in subsampled_bin])
		maxDivBC_std = cr_std([ maxDivBC[j] for j in subsampled_bin], maxDivBC_avg)
		maxDivBD_avg = cr_mean([ maxDivBD[j] for j in subsampled_bin])
		maxDivBD_std = cr_std([ maxDivBD[j] for j in subsampled_bin], maxDivBD_avg)
		maxDivCD_avg = cr_mean([ maxDivCD[j] for j in subsampled_bin])
		maxDivCD_std = cr_std([ maxDivCD[j] for j in subsampled_bin], maxDivCD_avg)
		
		GminAB_avg = cr_mean([ GminAB[j] for j in subsampled_bin])
		GminAB_std = cr_std([ GminAB[j] for j in subsampled_bin], GminAB_avg)
		GminAC_avg = cr_mean([ GminAC[j] for j in subsampled_bin])
		GminAC_std = cr_std([ GminAC[j] for j in subsampled_bin], GminAC_avg)
		GminAD_avg = cr_mean([ GminAD[j] for j in subsampled_bin])
		GminAD_std = cr_std([ GminAD[j] for j in subsampled_bin], GminAD_avg)
		GminBC_avg = cr_mean([ GminBC[j] for j in subsampled_bin])
		GminBC_std = cr_std([ GminBC[j] for j in subsampled_bin], GminBC_avg)
		GminBD_avg = cr_mean([ GminBD[j] for j in subsampled_bin])
		GminBD_std = cr_std([ GminBD[j] for j in subsampled_bin], GminBD_avg)
		GminCD_avg = cr_mean([ GminCD[j] for j in subsampled_bin])
		GminCD_std = cr_std([ GminCD[j] for j in subsampled_bin], GminCD_avg)
		
		GmaxAB_avg = cr_mean([ GmaxAB[j] for j in subsampled_bin])
		GmaxAB_std = cr_std([ GmaxAB[j] for j in subsampled_bin], GmaxAB_avg)
		GmaxAC_avg = cr_mean([ GmaxAC[j] for j in subsampled_bin])
		GmaxAC_std = cr_std([ GmaxAC[j] for j in subsampled_bin], GmaxAC_avg)
		GmaxAD_avg = cr_mean([ GmaxAD[j] for j in subsampled_bin])
		GmaxAD_std = cr_std([ GmaxAD[j] for j in subsampled_bin], GmaxAD_avg)
		GmaxBC_avg = cr_mean([ GmaxBC[j] for j in subsampled_bin])
		GmaxBC_std = cr_std([ GmaxBC[j] for j in subsampled_bin], GmaxBC_avg)
		GmaxBD_avg = cr_mean([ GmaxBD[j] for j in subsampled_bin])
		GmaxBD_std = cr_std([ GmaxBD[j] for j in subsampled_bin], GmaxBD_avg)
		GmaxCD_avg = cr_mean([ GmaxCD[j] for j in subsampled_bin])
		GmaxCD_std = cr_std([ GmaxCD[j] for j in subsampled_bin], GmaxCD_avg)

		FST_AB_avg = cr_mean([ FST_AB[j] for j in subsampled_bin])
		FST_AB_std = cr_std([ FST_AB[j] for j in subsampled_bin], FST_AB_avg)
		FST_AC_avg = cr_mean([ FST_AC[j] for j in subsampled_bin])
		FST_AC_std = cr_std([ FST_AC[j] for j in subsampled_bin], FST_AC_avg)
		FST_AD_avg = cr_mean([ FST_AD[j] for j in subsampled_bin])
		FST_AD_std = cr_std([ FST_AD[j] for j in subsampled_bin], FST_AD_avg)
		FST_BC_avg = cr_mean([ FST_BC[j] for j in subsampled_bin])
		FST_BC_std = cr_std([ FST_BC[j] for j in subsampled_bin], FST_BC_avg)
		FST_BD_avg = cr_mean([ FST_BD[j] for j in subsampled_bin])
		FST_BD_std = cr_std([ FST_BD[j] for j in subsampled_bin], FST_BD_avg)
		FST_CD_avg = cr_mean([ FST_CD[j] for j in subsampled_bin])
		FST_CD_std = cr_std([ FST_CD[j] for j in subsampled_bin], FST_CD_avg)
		
		D_AB_C_avg = cr_mean([ D_AB_C[j] for j in subsampled_bin])
		D_AB_C_std = cr_std([ D_AB_C[j] for j in subsampled_bin], D_AB_C_avg)
		D_AB_D_avg = cr_mean([ D_AB_D[j] for j in subsampled_bin])
		D_AB_D_std = cr_std([ D_AB_D[j] for j in subsampled_bin], D_AB_D_avg)
		D_BA_C_avg = cr_mean([ D_BA_C[j] for j in subsampled_bin])
		D_BA_C_std = cr_std([ D_BA_C[j] for j in subsampled_bin], D_BA_C_avg)
		D_BA_D_avg = cr_mean([ D_BA_D[j] for j in subsampled_bin])
		D_BA_D_std = cr_std([ D_BA_D[j] for j in subsampled_bin], D_BA_D_avg)
		D_CD_A_avg = cr_mean([ D_CD_A[j] for j in subsampled_bin])
		D_CD_A_std = cr_std([ D_CD_A[j] for j in subsampled_bin], D_CD_A_avg)
		D_CD_B_avg = cr_mean([ D_CD_B[j] for j in subsampled_bin])
		D_CD_B_std = cr_std([ D_CD_B[j] for j in subsampled_bin], D_CD_B_avg)
		D_DC_A_avg = cr_mean([ D_DC_A[j] for j in subsampled_bin])
		D_DC_A_std = cr_std([ D_DC_A[j] for j in subsampled_bin], D_DC_A_avg)
		D_DC_B_avg = cr_mean([ D_DC_B[j] for j in subsampled_bin])
		D_DC_B_std = cr_std([ D_DC_B[j] for j in subsampled_bin], D_DC_B_avg)
		
		fd_AB_C_avg = cr_mean([ fd_AB_C[j] for j in subsampled_bin])
		fd_AB_C_std = cr_std([ fd_AB_C[j] for j in subsampled_bin], fd_AB_C_avg)
		fd_AB_D_avg = cr_mean([ fd_AB_D[j] for j in subsampled_bin])
		fd_AB_D_std = cr_std([ fd_AB_D[j] for j in subsampled_bin], fd_AB_D_avg)
		fd_BA_C_avg = cr_mean([ fd_BA_C[j] for j in subsampled_bin])
		fd_BA_C_std = cr_std([ fd_BA_C[j] for j in subsampled_bin], fd_BA_C_avg)
		fd_BA_D_avg = cr_mean([ fd_BA_D[j] for j in subsampled_bin])
		fd_BA_D_std = cr_std([ fd_BA_D[j] for j in subsampled_bin], fd_BA_D_avg)
		fd_CD_A_avg = cr_mean([ fd_CD_A[j] for j in subsampled_bin])
		fd_CD_A_std = cr_std([ fd_CD_A[j] for j in subsampled_bin], fd_CD_A_avg)
		fd_CD_B_avg = cr_mean([ fd_CD_B[j] for j in subsampled_bin])
		fd_CD_B_std = cr_std([ fd_CD_B[j] for j in subsampled_bin], fd_CD_B_avg)
		fd_DC_A_avg = cr_mean([ fd_DC_A[j] for j in subsampled_bin])
		fd_DC_A_std = cr_std([ fd_DC_A[j] for j in subsampled_bin], fd_DC_A_avg)
		fd_DC_B_avg = cr_mean([ fd_DC_B[j] for j in subsampled_bin])
		fd_DC_B_std = cr_std([ fd_DC_B[j] for j in subsampled_bin], fd_DC_B_avg)
		
		fhom_AB_C_avg = cr_mean([ fhom_AB_C[j] for j in subsampled_bin])
		fhom_AB_C_std = cr_std([ fhom_AB_C[j] for j in subsampled_bin], fhom_AB_C_avg)
		fhom_AB_D_avg = cr_mean([ fhom_AB_D[j] for j in subsampled_bin])
		fhom_AB_D_std = cr_std([ fhom_AB_D[j] for j in subsampled_bin], fhom_AB_D_avg)
		fhom_BA_C_avg = cr_mean([ fhom_BA_C[j] for j in subsampled_bin])
		fhom_BA_C_std = cr_std([ fhom_BA_C[j] for j in subsampled_bin], fhom_BA_C_avg)
		fhom_BA_D_avg = cr_mean([ fhom_BA_D[j] for j in subsampled_bin])
		fhom_BA_D_std = cr_std([ fhom_BA_D[j] for j in subsampled_bin], fhom_BA_D_avg)
		fhom_CD_A_avg = cr_mean([ fhom_CD_A[j] for j in subsampled_bin])
		fhom_CD_A_std = cr_std([ fhom_CD_A[j] for j in subsampled_bin], fhom_CD_A_avg)
		fhom_CD_B_avg = cr_mean([ fhom_CD_B[j] for j in subsampled_bin])
		fhom_CD_B_std = cr_std([ fhom_CD_B[j] for j in subsampled_bin], fhom_CD_B_avg)
		fhom_DC_A_avg = cr_mean([ fhom_DC_A[j] for j in subsampled_bin])
		fhom_DC_A_std = cr_std([ fhom_DC_A[j] for j in subsampled_bin], fhom_DC_A_avg)
		fhom_DC_B_avg = cr_mean([ fhom_DC_B[j] for j in subsampled_bin])
		fhom_DC_B_std = cr_std([ fhom_DC_B[j] for j in subsampled_bin], fhom_DC_B_avg)
...
# new bpfile
outfile_bpfile = open('bpfile_{0}_bins'.format(subsampled), 'w')
outfile_bpfile.write(header_bpfile)
outfile_bpfile.write('\t'.join([str(L[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(nSamA[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(nSamB[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(nSamC[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(nSamD[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(theta[j]) for j in subsampled_bin ]))
outfile_bpfile.write('\n')
outfile_bpfile.write('\t'.join([str(rho[j]) for j in subsampled_bin ]))
